...
def home_page():
    ...
    out = """<html><head><title>MDM Test Console</title></head><body><form method="GET" action="/queue"><select name="command"><option value=''>Select command</option>%s</select><input type=submit value="Send"/></form><hr/><b>Last command sent</b><pre>%s</pre><hr/><b>Last result</b> (<a href="/">Refresh</a>)<pre>%s</pre></body></html>""" % (drop_list, last_sent, last_result)
    return out
